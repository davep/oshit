...
class ConfigurationDialog(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            ...
            yield Input(str(config.maximum_concurrency), id="max-con")
            yield Label("Connection Timeout:")
            yield Input(str(config.connection_timeout), id="timeout")
            yield Label("Maximum Top Items:")
            yield Input(str(config.maximum_top), id="max-top")
            yield Label("Maximum New Items:")
            yield Input(str(config.maximum_new), id="max-new")
            yield Label("Maximum Best Items:")
            yield Input(str(config.maximum_best), id="max-best")
            yield Label("Maximum Ask Items:")
            yield Input(str(config.maximum_ask), id="max-ask")
            yield Label("Maximum Show Items:")
            yield Input(str(config.maximum_show), id="max-show")
            yield Label("Maximum Jobs Items:")
            yield Input(str(config.maximum_jobs), id="max-jobs")
